...
Received: from alsa0.perex.cz (alsa0.perex.cz [77.48.224.243])
	by mail.lfdr.de (Postfix) with ESMTPS id 393E84EC705
	for <lists+alsa-devel@lfdr.de>; Wed, 30 Mar 2022 16:48:48 +0200 (CEST)
Received: from alsa1.perex.cz (alsa1.perex.cz [207.180.221.201])
	(using TLSv1.2 with cipher AECDH-AES256-SHA (256/256 bits))
	(No client certificate requested)
	by alsa0.perex.cz (Postfix) with ESMTPS id 9E4F81A9A;
	Wed, 30 Mar 2022 16:47:56 +0200 (CEST)
DKIM-Filter: OpenDKIM Filter v2.11.0 alsa0.perex.cz 9E4F81A9A
DKIM-Signature: v=1; a=rsa-sha256; c=relaxed/simple; d=alsa-project.org;
	s=default; t=1648651726;
	bh=pJWVYrqYaSpx98e/ZMj6Xp+VhdMU3WONPbIV1PcZyj8=;
	h=From:To:Subject:Date:In-Reply-To:References:Cc:List-Id:
	 List-Unsubscribe:List-Archive:List-Post:List-Help:List-Subscribe:
	 From;
	b=kox6PO3ZMAnGL2Jjg4gxSc6oI3mu0Lx5w8qCmTdxxJmDq1k9JwxVmZlGLBUt9Gite
	 80FWtTWxbHE2qD+MK9IMarAZw4/VI2MXgml0kEDZd0PeN8ICe9hubDd4LoTRxKDZxN
	 9EcXs93kOd9W+4GheZfuu7mnXv4EBos8x38kxhzw=
Received: from alsa1.perex.cz (localhost.localdomain [127.0.0.1])
	by alsa1.perex.cz (Postfix) with ESMTP id 15B14F80518;
	Wed, 30 Mar 2022 16:47:11 +0200 (CEST)
X-Original-To: alsa-devel@alsa-project.org
Delivered-To: alsa-devel@alsa-project.org
Received: by alsa1.perex.cz (Postfix, from userid 50401)
 id 03818F80517; Wed, 30 Mar 2022 16:47:06 +0200 (CEST)
...
Received: from n169-112.mail.139.com (n169-112.mail.139.com [120.232.169.112])
 ...
 by alsa1.perex.cz (Postfix) with ESMTPS id BDB7BF80254
 for <alsa-devel@alsa-project.org>; Wed, 30 Mar 2022 16:46:54 +0200 (CEST)
DKIM-Filter: OpenDKIM Filter v2.11.0 alsa1.perex.cz BDB7BF80254
X-RM-TagInfo: emlType=0                                       
X-RM-SPAM: 
X-RM-SPAM-FLAG: 00000000
Received: from LAPTOP-39QA2GAC.localdomain (unknown[223.73.211.113])
 by rmsmtp-lg-appmail-19-12022 (RichMail) with SMTP id 2ef662446d35717-f0abe;
 Wed, 30 Mar 2022 22:46:50 +0800 (CST)
X-RM-TRANSID: 2ef662446d35717-f0abe
From: Raphael-Xu <13691752556@139.com>
To: broonie@kernel.org
Subject: [PATCH v5 3/4] rename variables from tas2764 to tas27xx
Date: Wed, 30 Mar 2022 22:45:25 +0800
Message-Id: <20220330144526.498-3-13691752556@139.com>
...
@@ -36,43 +36,44 @@ struct tas2764_priv {
 	int i_sense_slot;
 };
...
@@ -81,81 +82,87 @@ static int tas2764_set_bias_level(struct snd_soc_component *component,
 }
...
@@ -166,25 +173,27 @@ static int tas2764_dac_event(struct snd_soc_dapm_widget *w,
 }
...
+static const struct snd_soc_dapm_route tas27xx_audio_map[] = {
 	...
 	{"ASI1 Sel", "Left", "ASI1"},
 	{"ASI1 Sel", "Right", "ASI1"},
@@ -195,14 +204,14 @@ static const struct snd_soc_dapm_route tas2764_audio_map[] = {
 	{"VSENSE", "Switch", "VMON"},
 };
...
+					    mute ? TAS27XX_PWR_CTRL_MUTE : 0);
 	...
 	if (ret < 0)
 		return ret;
@@ -210,9 +219,9 @@ static int tas2764_mute(struct snd_soc_dai *dai, int mute, int direction)
 	return 0;
 }
...
+	struct snd_soc_component *component = tas27xx->component;
 	...
 	int val;
 	int ret;
@@ -220,21 +229,21 @@ static int tas2764_set_bitwidth(struct tas2764_priv *tas2764, int bitwidth)
 	switch (bitwidth) {
...
+					TAS27XX_TDM_CFG2_RXW_32BITS);
 	...
 	default:
@@ -244,65 +253,64 @@ static int tas2764_set_bitwidth(struct tas2764_priv *tas2764, int bitwidth)
 	if (ret < 0)
 		return ret;
...
+					    TAS27XX_TDM_CFG0_MASK,
 	...
 	if (ret < 0)
 		return ret;
@@ -310,43 +318,45 @@ static int tas2764_set_samplerate(struct tas2764_priv *tas2764, int samplerate)
 	return 0;
 }
...
+					    TAS27XX_TDM_CFG1_RX_MASK,
 	...
 	if (ret < 0)
 		return ret;
@@ -354,41 +364,42 @@ static int tas2764_set_fmt(struct snd_soc_dai *dai, unsigned int fmt)
...
+		snd_soc_component_get_drvdata(component);
 	...
 	int slots_cfg;
 	int slot_size;
@@ -416,159 +427,162 @@ static int tas2764_set_dai_tdm_slot(struct snd_soc_dai *dai,
...
+		.selector_reg = TAS27XX_PAGE,
 		...
 		.selector_shift = 0,
 		.window_start = 0,
@@ -576,113 +590,113 @@ static const struct regmap_range_cfg tas2764_regmap_ranges[] = {
 	},
 };
