...
Received: from alsa0.perex.cz (alsa0.perex.cz [77.48.224.243])
	by mail.lfdr.de (Postfix) with ESMTPS id B1D3B170EBF
	for <lists+alsa-devel@lfdr.de>; Thu, 27 Feb 2020 03:52:16 +0100 (CET)
Received: from alsa1.perex.cz (alsa1.perex.cz [207.180.221.201])
	(using TLSv1.2 with cipher AECDH-AES256-SHA (256/256 bits))
	(No client certificate requested)
	by alsa0.perex.cz (Postfix) with ESMTPS id 19BD716A3;
	Thu, 27 Feb 2020 03:51:26 +0100 (CET)
DKIM-Filter: OpenDKIM Filter v2.11.0 alsa0.perex.cz 19BD716A3
DKIM-Signature: v=1; a=rsa-sha256; c=relaxed/simple; d=alsa-project.org;
	s=default; t=1582771936;
	bh=h6nSLlzWhoN9YpGD+0Lj2obKQuGTarSMEDUqyrUb5Gg=;
	h=From:To:Subject:Date:In-Reply-To:References:In-Reply-To:
	 References:Cc:List-Id:List-Unsubscribe:List-Archive:List-Post:
	 List-Help:List-Subscribe:From;
	b=i0PzrUP1IaDJN3VxfRmINMQq/9rdazEBMZDkLwyvqZW+rRNa6qkqWQV0OxvM8+1tE
	 Dfkw2wQ+yOVeDAlh3x0MnWQYNIGTmObk3XKLryZhH0nFJS7rWFW8FB/3vSth5mKoKJ
	 b9yUpOhhlJhYeF7F2hZzgrumll05tGDXTHeinuh8=
Received: from alsa1.perex.cz (localhost.localdomain [127.0.0.1])
	by alsa1.perex.cz (Postfix) with ESMTP id B6256F802A0;
	Thu, 27 Feb 2020 03:48:23 +0100 (CET)
X-Original-To: alsa-devel@alsa-project.org
Delivered-To: alsa-devel@alsa-project.org
Received: by alsa1.perex.cz (Postfix, from userid 50401)
 id E6265F80291; Thu, 27 Feb 2020 03:48:20 +0100 (CET)
...
Received: from inva021.nxp.com (inva021.nxp.com [92.121.34.21])
 ...
 (No client certificate requested)
 by alsa1.perex.cz (Postfix) with ESMTPS id A4C78F80161
 for <alsa-devel@alsa-project.org>; Thu, 27 Feb 2020 03:48:08 +0100 (CET)
DKIM-Filter: OpenDKIM Filter v2.11.0 alsa1.perex.cz A4C78F80161
Received: from inva021.nxp.com (localhost [127.0.0.1])
 by inva021.eu-rdc02.nxp.com (Postfix) with ESMTP id 2D5F2208DC4;
 Thu, 27 Feb 2020 03:48:08 +0100 (CET)
Received: from invc005.ap-rdc01.nxp.com (invc005.ap-rdc01.nxp.com
 [165.114.16.14])
 by inva021.eu-rdc02.nxp.com (Postfix) with ESMTP id D201D208DC1;
 Thu, 27 Feb 2020 03:47:59 +0100 (CET)
Received: from localhost.localdomain (shlinux2.ap.freescale.net
 [10.192.224.44])
 by invc005.ap-rdc01.nxp.com (Postfix) with ESMTP id 5A8384030D;
 Thu, 27 Feb 2020 10:47:50 +0800 (SGT)
From: Shengjiu Wang <shengjiu.wang@nxp.com>
To: timur@kernel.org, nicoleotsuka@gmail.com, Xiubo.Lee@gmail.com,
 festevam@gmail.com, broonie@kernel.org, alsa-devel@alsa-project.org,
 lgirdwood@gmail.com, perex@perex.cz, tiwai@suse.com, robh+dt@kernel.org,
 mark.rutland@arm.com, devicetree@vger.kernel.org
Subject: [PATCH v3 4/4] ASoC: fsl_easrc: Add EASRC ASoC CPU DAI and platform
 drivers
Date: Thu, 27 Feb 2020 10:41:58 +0800
Message-Id: <356c7c47435ffce7b52113dbafec0cbdbd88b1ea.1582770784.git.shengjiu.wang@nxp.com>
...
index 000000000000..55a2204f5bfb
--- /dev/null
...
+	struct fsl_asrc *asrc_priv = snd_soc_component_get_drvdata(comp);
+	struct fsl_easrc_internal *easrc_internal = asrc_priv->private;
+	struct soc_mreg_control *mc =
+		(struct soc_mreg_control *)kcontrol->private_value;
+	unsigned int regval = ucontrol->value.integer.value[0];
+
+	easrc_internal->bps_iec958[mc->regbase] = regval;
+
+	return 0;
+}
+
...
+		(struct soc_mreg_control *)kcontrol->private_value;
+
+	ucontrol->value.enumerated.item[0] = easrc_internal->bps_iec958[mc->regbase];
+
+	return 0;
+}
+
...
+{
+	struct fsl_asrc *asrc_priv = ctx->asrc_priv;
+	struct fsl_easrc_internal *easrc_internal = asrc_priv->private;
+	struct fsl_easrc_ctx_internal *ctx_internal = ctx->private;
+	unsigned int in_rate = ctx_internal->in_params.norm_rate;
+	unsigned int out_rate = ctx_internal->out_params.norm_rate;
+	unsigned int int_bits;
+	unsigned int frac_bits;
+	u64 val;
+	u32 *r;
+
+	switch (easrc_internal->rs_num_taps) {
...
+	do_div(val, out_rate);
+	r = (uint32_t *)&val;
+	regmap_write(asrc_priv->regmap, REG_EASRC_RRL(ctx->index),
+		     EASRC_RRL_RS_RL(r[0]));
+	regmap_write(asrc_priv->regmap, REG_EASRC_RRH(ctx->index),
+		     EASRC_RRH_RS_RH(r[1]));
+
+	return 0;
+}
...
+{
+	struct fsl_easrc_ctx_internal *ctx_internal;
+	int a, b;
+
+	if (!ctx)
+		return;
+
+	ctx_internal = ctx->private;
+
+	a = ctx_internal->in_params.sample_rate;
+	b = ctx_internal->out_params.sample_rate;
+
+	a = gcd(a, b);
+
+	/* Divide by gcd to normalize the rate */
+	ctx_internal->in_params.norm_rate = ctx_internal->in_params.sample_rate / a;
+	ctx_internal->out_params.norm_rate = ctx_internal->out_params.sample_rate / a;
+}
+
+/* Resets the pointer of the coeff memory pointers */
+static int fsl_easrc_coeff_mem_ptr_reset(struct fsl_asrc *asrc_priv,
...
+	 * 0x0 to 0x1 to 0x0.
+	 */
+	regmap_update_bits(asrc_priv->regmap, reg, mask, 0);
+	regmap_update_bits(asrc_priv->regmap, reg, mask, val);
+	regmap_update_bits(asrc_priv->regmap, reg, mask, 0);
+
+	return 0;
+}
...
+}
+
+static int fsl_easrc_resampler_config(struct fsl_asrc *asrc_priv)
+{
+	struct device *dev = &asrc_priv->pdev->dev;
+	struct fsl_easrc_internal *easrc_internal = asrc_priv->private;
+	struct asrc_firmware_hdr *hdr =  easrc_internal->firmware_hdr;
+	struct interp_params *interp = easrc_internal->interp;
...
+		    bits_taps_to_val(easrc_internal->rs_num_taps)) {
+			arr = interp[i].coeff;
...
+	regmap_write(asrc_priv->regmap, REG_EASRC_RCTCL, EASRC_RCTCL_RS_CL(r[0]));
+	regmap_write(asrc_priv->regmap, REG_EASRC_RCTCH, EASRC_RCTCH_RS_CH(r[1]));
+
+	/*
...
+	 * 11b - N/A
+	 */
+	regmap_update_bits(asrc_priv->regmap, REG_EASRC_CRCC,
+			   EASRC_CRCC_RS_TAPS_MASK,
+			   EASRC_CRCC_RS_TAPS(easrc_internal->rs_num_taps));
+
+	/* Reset prefilter coefficient pointer back to 0 */
+	ret = fsl_easrc_coeff_mem_ptr_reset(asrc_priv, 0, EASRC_RS_COEFF_MEM);
+	if (ret)
+		return ret;
+
+	/*
...
+		r = (uint32_t *)&arr[i];
+		regmap_write(asrc_priv->regmap, REG_EASRC_CRCM,
...
+ */
+static int fsl_easrc_normalize_filter(struct fsl_asrc *asrc_priv,
+				      u64 *infilter,
+				      u64 *outfilter,
+				      int shift)
+{
...
+}
+
+static int fsl_easrc_write_pf_coeff_mem(struct fsl_asrc *asrc_priv, int ctx_id,
...
+	 */
+	ret = fsl_easrc_coeff_mem_ptr_reset(asrc_priv, ctx_id, EASRC_PF_COEFF_MEM);
+	if (ret)
+		return ret;
+
+	for (i = 0; i < (n_taps + 1) / 2; i++) {
+		ret = fsl_easrc_normalize_filter(asrc_priv, &arr[i], &tmp, shift);
+		if (ret)
+			return ret;
+
+		r = (uint32_t *)&tmp;
+		regmap_write(asrc_priv->regmap, REG_EASRC_PCF(ctx_id),
+			     EASRC_PCF_CD(r[0]));
+		regmap_write(asrc_priv->regmap, REG_EASRC_PCF(ctx_id),
+			     EASRC_PCF_CD(r[1]));
+	}
...
+}
+
+static int fsl_easrc_prefilter_config(struct fsl_asrc *asrc_priv,
+				      unsigned int ctx_id)
+{
+	struct prefil_params *prefil, *selected_prefil = NULL;
+	struct fsl_easrc_ctx_internal *ctx_internal;
+	struct fsl_easrc_internal *easrc_internal;
...
+	}
+
+	easrc_internal = asrc_priv->private;
+
+	ctx = asrc_priv->pair[ctx_id];
+	ctx_internal = ctx->private;
+
+	ctx_internal->in_filled_sample = bits_taps_to_val(easrc_internal->rs_num_taps) / 2;
+	ctx_internal->out_missed_sample = ctx_internal->in_filled_sample *
+					  ctx_internal->out_params.sample_rate /
+					  ctx_internal->in_params.sample_rate;
+
+	ctx_internal->st1_num_taps = 0;
+	ctx_internal->st2_num_taps = 0;
+
+	regmap_write(asrc_priv->regmap, REG_EASRC_CCE1(ctx_id), 0);
+	regmap_write(asrc_priv->regmap, REG_EASRC_CCE2(ctx_id), 0);
+
+	/*
...
+	 *
+	 */
+	if (ctx_internal->out_params.sample_rate >= ctx_internal->in_params.sample_rate) {
+		if (ctx_internal->out_params.sample_rate == ctx_internal->in_params.sample_rate)
+			regmap_update_bits(asrc_priv->regmap,
+					   REG_EASRC_CCE1(ctx_id),
+					   EASRC_CCE1_RS_BYPASS_MASK,
+					   EASRC_CCE1_RS_BYPASS);
+
+		ctx_internal->st1_num_taps = 1;
+		ctx_internal->st1_coeff    = &easrc_internal->const_coeff;
+		ctx_internal->st1_num_exp  = 1;
+		ctx_internal->st2_num_taps = 0;
+
+		if (ctx_internal->in_params.sample_format == SNDRV_PCM_FORMAT_FLOAT_LE &&
+		    ctx_internal->out_params.sample_format != SNDRV_PCM_FORMAT_FLOAT_LE)
+			ctx_internal->st1_addexp = 31;
+		else if (ctx_internal->in_params.sample_format != SNDRV_PCM_FORMAT_FLOAT_LE &&
+			 ctx_internal->out_params.sample_format == SNDRV_PCM_FORMAT_FLOAT_LE)
+			ctx_internal->st1_addexp -= ctx_internal->in_params.fmt.addexp;
+	} else {
+		inrate = ctx_internal->in_params.norm_rate;
+		outrate = ctx_internal->out_params.norm_rate;
+
+		hdr = easrc_internal->firmware_hdr;
+		prefil = easrc_internal->prefil;
+
...
+				ctx_internal->in_params.sample_rate, inrate,
+				ctx_internal->out_params.sample_rate, outrate);
+			return -EINVAL;
+		}
+
+		/*
...
+		 */
+		ctx_internal->st1_num_taps = selected_prefil->st1_taps;
+		ctx_internal->st1_coeff    = selected_prefil->coeff;
+		ctx_internal->st1_num_exp  = selected_prefil->st1_exp;
+
+		offset = ((selected_prefil->st1_taps + 1) / 2);
+		ctx_internal->st2_num_taps = selected_prefil->st2_taps;
+		ctx_internal->st2_coeff    = selected_prefil->coeff + offset;
+
+		if (ctx_internal->in_params.sample_format == SNDRV_PCM_FORMAT_FLOAT_LE &&
+		    ctx_internal->out_params.sample_format != SNDRV_PCM_FORMAT_FLOAT_LE) {
+			/* only change stage2 coefficient for 2 stage case */
+			if (ctx_internal->st2_num_taps > 0)
+				ctx_internal->st2_addexp = 31;
+			else
+				ctx_internal->st1_addexp = 31;
+		} else if (ctx_internal->in_params.sample_format != SNDRV_PCM_FORMAT_FLOAT_LE &&
+			   ctx_internal->out_params.sample_format == SNDRV_PCM_FORMAT_FLOAT_LE) {
+			if (ctx_internal->st2_num_taps > 0)
+				ctx_internal->st2_addexp -= ctx_internal->in_params.fmt.addexp;
+			else
+				ctx_internal->st1_addexp -= ctx_internal->in_params.fmt.addexp;
+		}
+	}
+
+	ctx_internal->in_filled_sample += (ctx_internal->st1_num_taps / 2) * ctx_internal->st1_num_exp +
+				  ctx_internal->st2_num_taps / 2;
+	ctx_internal->out_missed_sample = ctx_internal->in_filled_sample *
+				 ctx_internal->out_params.sample_rate /
+				 ctx_internal->in_params.sample_rate;
+
+	if (ctx_internal->in_filled_sample * ctx_internal->out_params.sample_rate %
+				ctx_internal->in_params.sample_rate != 0)
+		ctx_internal->out_missed_sample += 1;
+	/*
...
+	 */
+	regmap_update_bits(asrc_priv->regmap, REG_EASRC_CC(ctx_id),
+			   EASRC_CC_EN_MASK, 0);
+
+	if (ctx_internal->st1_num_taps > EASRC_MAX_PF_TAPS) {
+		dev_err(dev, "ST1 taps [%d] mus be lower than %d\n",
+			ctx_internal->st1_num_taps, EASRC_MAX_PF_TAPS);
+		ret = -EINVAL;
+		goto ctx_error;
+	}
+
+	/* Update ctx ST1_NUM_TAPS in Context Control Extended 2 register */
+	regmap_update_bits(asrc_priv->regmap, REG_EASRC_CCE2(ctx_id),
+			   EASRC_CCE2_ST1_TAPS_MASK,
+			   EASRC_CCE2_ST1_TAPS(ctx_internal->st1_num_taps - 1));
+
+	/* Prefilter Coefficient Write Select to write in ST1 coeff */
+	regmap_update_bits(asrc_priv->regmap, REG_EASRC_CCE1(ctx_id),
+			   EASRC_CCE1_COEF_WS_MASK,
+			   EASRC_PF_ST1_COEFF_WR << EASRC_CCE1_COEF_WS_SHIFT);
+
+	ret = fsl_easrc_write_pf_coeff_mem(asrc_priv, ctx_id,
+					   ctx_internal->st1_coeff,
+					   ctx_internal->st1_num_taps,
+					   ctx_internal->st1_addexp);
+	if (ret)
+		goto ctx_error;
+
+	if (ctx_internal->st2_num_taps > 0) {
+		if (ctx_internal->st2_num_taps + ctx_internal->st1_num_taps > EASRC_MAX_PF_TAPS) {
...
+				   EASRC_CCE1_PF_EXP(ctx_internal->st1_num_exp - 1));
+
+		/* Update ctx ST2_NUM_TAPS in Context Control Extended 2 reg */
+		regmap_update_bits(asrc_priv->regmap, REG_EASRC_CCE2(ctx_id),
+				   EASRC_CCE2_ST2_TAPS_MASK,
+				   EASRC_CCE2_ST2_TAPS(ctx_internal->st2_num_taps - 1));
+
...
+
+		ret = fsl_easrc_write_pf_coeff_mem(asrc_priv, ctx_id,
+						   ctx_internal->st2_coeff,
+						   ctx_internal->st2_num_taps,
+						   ctx_internal->st2_addexp);
+		if (ret)
+			goto ctx_error;
+	}
+
...
+	int channels = 0;
+
+	if (ctx_internal->st1_num_taps > 0) {
+		if (ctx_internal->st2_num_taps > 0)
+			st1_mem_alloc =
+				(ctx_internal->st1_num_taps - 1) * ctx_internal->st1_num_exp + 1;
+		else
+			st1_mem_alloc = ctx_internal->st1_num_taps;
+	}
+
+	if (ctx_internal->st2_num_taps > 0)
+		st2_mem_alloc = ctx_internal->st2_num_taps;
+
...
+
+	regmap_update_bits(asrc_priv->regmap, reg0,
...
+		if (ctx_internal->st2_num_taps > 0)
+			st1_mem_alloc =
+				(ctx_internal->st1_num_taps - 1) * slot->num_channel *
+				ctx_internal->st1_num_exp + slot->num_channel;
+		else
+			st1_mem_alloc = ctx_internal->st1_num_taps * slot->num_channel;
+
+		slot->pf_mem_used = st1_mem_alloc;
+		regmap_update_bits(asrc_priv->regmap, reg2,
...
+
+	if (ctx_internal->st2_num_taps > 0) {
+		st1_chanxexp = slot->num_channel * (ctx_internal->st1_num_exp - 1);
+
+		regmap_update_bits(asrc_priv->regmap, reg1,
+				   EASRC_DPCS0R1_ST1_EXP_MASK,
+				   EASRC_DPCS0R1_ST1_EXP(st1_chanxexp));
+
+		st2_mem_alloc = slot->num_channel * ctx_internal->st2_num_taps;
+		slot->pf_mem_used += st2_mem_alloc;
+		regmap_update_bits(asrc_priv->regmap, reg3,
...
+ *
+ */
+static int fsl_easrc_config_slot(struct fsl_asrc *asrc_priv, unsigned int ctx_id)
+{
+	struct fsl_easrc_internal *easrc_internal = asrc_priv->private;
+	struct fsl_asrc_pair *ctx = asrc_priv->pair[ctx_id];
...
+		slot0 = &easrc_internal->slot[i][0];
+		slot1 = &easrc_internal->slot[i][1];
+
...
+
+	if (req_channels > 0) {
+		dev_err(&asrc_priv->pdev->dev, "no avail slot.\n");
+		return -EINVAL;
+	}
+
+	return 0;
...
+ */
+static int fsl_easrc_release_slot(struct fsl_asrc *asrc_priv, unsigned int ctx_id)
+{
+	struct fsl_easrc_internal *easrc_internal = asrc_priv->private;
+	struct fsl_asrc_pair *ctx = asrc_priv->pair[ctx_id];
+	int i;
+
+	for (i = 0; i < EASRC_CTX_MAX_NUM; i++) {
+		if (easrc_internal->slot[i][0].busy &&
+		    easrc_internal->slot[i][0].ctx_index == ctx->index) {
+			easrc_internal->slot[i][0].busy = false;
+			easrc_internal->slot[i][0].num_channel = 0;
+			easrc_internal->slot[i][0].pf_mem_used = 0;
+			/* set registers */
+			regmap_write(asrc_priv->regmap, REG_EASRC_DPCS0R0(i), 0);
+			regmap_write(asrc_priv->regmap, REG_EASRC_DPCS0R1(i), 0);
+			regmap_write(asrc_priv->regmap, REG_EASRC_DPCS0R2(i), 0);
+			regmap_write(asrc_priv->regmap, REG_EASRC_DPCS0R3(i), 0);
+		}
+
+		if (easrc_internal->slot[i][1].busy &&
+		    easrc_internal->slot[i][1].ctx_index == ctx->index) {
+			easrc_internal->slot[i][1].busy = false;
+			easrc_internal->slot[i][1].num_channel = 0;
+			easrc_internal->slot[i][1].pf_mem_used = 0;
+			/* set registers */
+			regmap_write(asrc_priv->regmap, REG_EASRC_DPCS1R0(i), 0);
+			regmap_write(asrc_priv->regmap, REG_EASRC_DPCS1R1(i), 0);
+			regmap_write(asrc_priv->regmap, REG_EASRC_DPCS1R2(i), 0);
+			regmap_write(asrc_priv->regmap, REG_EASRC_DPCS1R3(i), 0);
+		}
+	}
+
+	return 0;
...
+ */
+int fsl_easrc_config_context(struct fsl_asrc *asrc_priv, unsigned int ctx_id)
+{
+	struct fsl_easrc_ctx_internal *ctx_internal;
+	struct fsl_asrc_pair *ctx;
+	struct device *dev;
...
+	ret = fsl_easrc_prefilter_config(asrc_priv, ctx->index);
+	if (ret)
+		return ret;
+
+	spin_lock_irqsave(&asrc_priv->lock, lock_flags);
+	ret = fsl_easrc_config_slot(asrc_priv, ctx->index);
...
+			   EASRC_CCE1_RS_INIT(ctx_internal->rs_init_mode));
+
+	regmap_update_bits(asrc_priv->regmap, REG_EASRC_CCE1(ctx_id),
+			   EASRC_CCE1_PF_INIT_MASK,
+			   EASRC_CCE1_PF_INIT(ctx_internal->pf_init_mode));
+
+	/* Context Input FIFO Watermark */
+	regmap_update_bits(asrc_priv->regmap, REG_EASRC_CC(ctx_id),
+			   EASRC_CC_FIFO_WTMK_MASK,
+			   EASRC_CC_FIFO_WTMK(ctx_internal->in_params.fifo_wtmk));
+
+	/* Context Output FIFO Watermark */
+	regmap_update_bits(asrc_priv->regmap, REG_EASRC_COC(ctx_id),
+			   EASRC_COC_FIFO_WTMK_MASK,
+			   EASRC_COC_FIFO_WTMK(ctx_internal->out_params.fifo_wtmk - 1));
+
+	/* Number of channels */
+	regmap_update_bits(asrc_priv->regmap, REG_EASRC_CC(ctx_id),
+			   EASRC_CC_CHEN_MASK,
...
+		fmt->width = easrc_internal->bps_iec958[ctx->index];
+		fmt->iec958 = 1;
...
+	struct fsl_asrc *asrc_priv = ctx->asrc_priv;
+	struct fsl_easrc_ctx_internal *ctx_internal = ctx->private;
+	struct fsl_easrc_data_fmt *in_fmt = &ctx_internal->in_params.fmt;
+	struct fsl_easrc_data_fmt *out_fmt = &ctx_internal->out_params.fmt;
+	int ret;
+
...
+	}
+
+	regmap_update_bits(asrc_priv->regmap, REG_EASRC_CC(ctx->index),
+			   EASRC_CC_BPS_MASK,
...
+	}
+
+	regmap_update_bits(asrc_priv->regmap, REG_EASRC_COC(ctx->index),
+			   EASRC_COC_BPS_MASK,
...
+		return -ENODEV;
+
+	asrc_priv = ctx->asrc_priv;
+	ctx_internal = ctx->private;
+	dev = &asrc_priv->pdev->dev;
+
+	/* input interleaving parameters */
+	regmap_update_bits(asrc_priv->regmap, REG_EASRC_CIA(ctx->index),
+			   EASRC_CIA_ITER_MASK,
+			   EASRC_CIA_ITER(ctx_internal->in_params.iterations));
+	regmap_update_bits(asrc_priv->regmap, REG_EASRC_CIA(ctx->index),
+			   EASRC_CIA_GRLEN_MASK,
+			   EASRC_CIA_GRLEN(ctx_internal->in_params.group_len));
+	regmap_update_bits(asrc_priv->regmap, REG_EASRC_CIA(ctx->index),
+			   EASRC_CIA_ACCLEN_MASK,
+			   EASRC_CIA_ACCLEN(ctx_internal->in_params.access_len));
+
+	/* output interleaving parameters */
+	regmap_update_bits(asrc_priv->regmap, REG_EASRC_COA(ctx->index),
+			   EASRC_COA_ITER_MASK,
+			   EASRC_COA_ITER(ctx_internal->out_params.iterations));
+	regmap_update_bits(asrc_priv->regmap, REG_EASRC_COA(ctx->index),
+			   EASRC_COA_GRLEN_MASK,
+			   EASRC_COA_GRLEN(ctx_internal->out_params.group_len));
+	regmap_update_bits(asrc_priv->regmap, REG_EASRC_COA(ctx->index),
+			   EASRC_COA_ACCLEN_MASK,
+			   EASRC_COA_ACCLEN(ctx_internal->out_params.access_len));
+
+	return 0;
+}
+
...
+		ret = -EBUSY;
+	} else if (channels > asrc_priv->channel_avail) {
...
+	spin_lock_irqsave(&asrc_priv->lock, lock_flags);
+
+	ret = fsl_easrc_release_slot(asrc_priv, ctx->index);
+
+	asrc_priv->channel_avail += ctx->channels;
+	asrc_priv->pair[ctx->index] = NULL;
+
+	spin_unlock_irqrestore(&asrc_priv->lock, lock_flags);
+}
+
...
+	int retry = 200;
+
+	regmap_read(asrc_priv->regmap, REG_EASRC_CC(ctx->index), &val);
+
...
+		do {
+			regmap_read(asrc_priv->regmap, REG_EASRC_SFS(ctx->index), &val);
+			val &= EASRC_SFS_NSGO_MASK;
+			size = val >> EASRC_SFS_NSGO_SHIFT;
+
+			/* Read FIFO, drop the data */
+			for (i = 0; i < size * ctx->channels; i++)
+				regmap_read(asrc_priv->regmap, REG_EASRC_RDFIFO(ctx->index), &val);
+			/* Check RUN_STOP_DONE */
+			regmap_read(asrc_priv->regmap, REG_EASRC_IRQF, &val);
+			if (val & EASRC_IRQF_RSD(1 << ctx->index)) {
+				/*Clear RUN_STOP_DONE*/
+				regmap_write_bits(asrc_priv->regmap,
+						  REG_EASRC_IRQF,
...
+
+		if (retry == 0)
+			dev_warn(&asrc_priv->pdev->dev, "RUN STOP fail\n");
+	}
+
+	regmap_update_bits(asrc_priv->regmap, REG_EASRC_CC(ctx->index),
...
+	}
+
+	ctx_internal->ctx_streams |= BIT(substream->stream);
+
+	/*
...
+		ctx_internal->in_params.sample_rate = rate;
+		ctx_internal->in_params.sample_format = format;
+		ctx_internal->out_params.sample_rate = asrc_priv->asrc_rate;
+		ctx_internal->out_params.sample_format = asrc_priv->asrc_format;
+	} else {
+		ctx_internal->out_params.sample_rate = rate;
+		ctx_internal->out_params.sample_format = format;
+		ctx_internal->in_params.sample_rate = asrc_priv->asrc_rate;
+		ctx_internal->in_params.sample_format = asrc_priv->asrc_format;
+	}
+
+	ctx->channels = channels;
+	ctx_internal->in_params.fifo_wtmk  = 0x20;
+	ctx_internal->out_params.fifo_wtmk = 0x20;
+
+	/*
...
+				       &ctx_internal->in_params.sample_format,
+				       &ctx_internal->out_params.sample_format);
+	if (ret) {
+		dev_err(dev, "failed to set format %d", ret);
+		return ret;
+	}
+
+	ret = fsl_easrc_config_context(asrc_priv, ctx->index);
+	if (ret) {
+		dev_err(dev, "failed to config context\n");
+		return ret;
+	}
+
+	ctx_internal->in_params.iterations = 1;
+	ctx_internal->in_params.group_len = ctx->channels;
+	ctx_internal->in_params.access_len = ctx->channels;
+	ctx_internal->out_params.iterations = 1;
+	ctx_internal->out_params.group_len = ctx->channels;
+	ctx_internal->out_params.access_len = ctx->channels;
+
...
+	struct fsl_easrc_ctx_internal *ctx_internal = ctx->private;
+
+	if (ctx && (ctx_internal->ctx_streams & BIT(substream->stream))) {
+		ctx_internal->ctx_streams &= ~BIT(substream->stream);
...
+{
+	struct fsl_asrc *asrc_priv = dev_get_drvdata(cpu_dai->dev);
+
+	snd_soc_dai_init_dma_data(cpu_dai,
+				  &asrc_priv->dma_params_tx,
...
+};
+
+void fsl_easrc_dump_firmware(struct fsl_asrc *asrc_priv)
+{
+	struct fsl_easrc_internal *easrc_internal = asrc_priv->private;
+	struct asrc_firmware_hdr *firm = easrc_internal->firmware_hdr;
+	struct interp_params *interp = easrc_internal->interp;
+	struct prefil_params *prefil = easrc_internal->prefil;
+	struct device *dev = &asrc_priv->pdev->dev;
+	int i;
+
...
+}
+
+int fsl_easrc_get_firmware(struct fsl_asrc *asrc_priv)
+{
+	struct fsl_easrc_internal *easrc_internal;
...
+	easrc_internal = asrc_priv->private;
+
+	ret = request_firmware(&easrc_internal->fw, easrc_internal->fw_name,
+			       &asrc_priv->pdev->dev);
+	if (ret)
+		return ret;
+
+	easrc_internal->firmware_hdr = (struct asrc_firmware_hdr *)easrc_internal->fw->data;
+	pnum = easrc_internal->firmware_hdr->prefil_scen;
+	inum = easrc_internal->firmware_hdr->interp_scen;
+
+	if (inum) {
+		offset = sizeof(struct asrc_firmware_hdr);
+		easrc_internal->interp =
+			(struct interp_params *)(easrc_internal->fw->data + offset);
+	}
+
+	if (pnum) {
+		offset = sizeof(struct asrc_firmware_hdr) +
+				inum * sizeof(struct interp_params);
+		easrc_internal->prefil =
+			(struct prefil_params *)(easrc_internal->fw->data + offset);
+	}
+
+	return 0;
+}
...
+		return -ENOMEM;
+
+	easrc_internal = devm_kzalloc(&pdev->dev, sizeof(*easrc_internal), GFP_KERNEL);
+	if (!easrc_internal)
+		return -ENOMEM;
+
+	asrc_priv->pdev = pdev;
+	asrc_priv->private = easrc_internal;
+	np = pdev->dev.of_node;
+
...
+
+	/* Set default value */
+	asrc_priv->channel_avail = 32;
+	asrc_priv->get_dma_channel = fsl_easrc_get_dma_channel;
+	asrc_priv->request_pair = fsl_easrc_request_context;
+	asrc_priv->release_pair = fsl_easrc_release_context;
+	asrc_priv->get_fifo_addr = fsl_easrc_get_fifo_addr;
+
+	easrc_internal->rs_num_taps = EASRC_RS_128_TAPS;
+	easrc_internal->const_coeff = 0x3FF0000000000000;
+
+	ret = of_property_read_u32(np, "fsl,asrc-rate", &asrc_priv->asrc_rate);
+	if (ret) {
...
+	else
+		asrc_priv->asrc_format = SNDRV_PCM_FORMAT_S32_LE;
+
...
+
+	spin_lock_irqsave(&asrc_priv->lock, lock_flags);
+	easrc_internal->firmware_loaded = 0;
+	spin_unlock_irqrestore(&asrc_priv->lock, lock_flags);
+
+	return 0;
+}
...
+
+	spin_lock_irqsave(&asrc_priv->lock, lock_flags);
+	if (easrc_internal->firmware_loaded) {
+		spin_unlock_irqrestore(&asrc_priv->lock, lock_flags);
+		goto skip_load;
+	}
+	easrc_internal->firmware_loaded = 1;
+	spin_unlock_irqrestore(&asrc_priv->lock, lock_flags);
+
+	ret = fsl_easrc_get_firmware(asrc_priv);
+	if (ret) {
...
+	 */
+	ret = fsl_easrc_resampler_config(asrc_priv);
+	if (ret) {
...
+				ctx_internal->out_missed_sample += 1;
+
+			ret = fsl_easrc_write_pf_coeff_mem(asrc_priv, i,
+							   ctx_internal->st1_coeff,
+							   ctx_internal->st1_num_taps,
+							   ctx_internal->st1_addexp);
+			if (ret)
...
new file mode 100644
index 000000000000..71120ffca997
--- /dev/null
...
+
+/**
+ * fsl_easrc_ctx_internal: EASRC context private data
+ *
...
+
+/**
+ * fsl_easrc_internal: EASRC private data
+ *
+ * @slot: slot setting
