...
Received: from alsa0.perex.cz (alsa0.perex.cz [77.48.224.243])
	by mail.lfdr.de (Postfix) with ESMTPS id 81FB576FDA9
	for <lists+alsa-devel@lfdr.de>; Fri,  4 Aug 2023 11:44:59 +0200 (CEST)
Received: from alsa1.perex.cz (alsa1.perex.cz [207.180.221.201])
	(using TLSv1.2 with cipher ADH-AES256-GCM-SHA384 (256/256 bits))
	(No client certificate requested)
	by alsa0.perex.cz (Postfix) with ESMTPS id 86A4B86F;
	Fri,  4 Aug 2023 11:44:08 +0200 (CEST)
DKIM-Filter: OpenDKIM Filter v2.11.0 alsa0.perex.cz 86A4B86F
DKIM-Signature: v=1; a=rsa-sha256; c=relaxed/simple; d=alsa-project.org;
	s=default; t=1691142298;
	bh=wIL9BYzk74UcpjUo3hUNvhoX4mZaa+gQwiWtkaBxHp4=;
	h=From:To:CC:Subject:Date:In-Reply-To:References:List-Id:
	 List-Archive:List-Help:List-Owner:List-Post:List-Subscribe:
	 List-Unsubscribe:From;
	b=useK58hhRakFpeF3R3BL2vY6wA/FtyYr+DCYT9rfULE8q6J1PPZVfUt6lVeV5CQh1
	 XHwbfobPgxcdlb+WHl/pe+tD53dQGQ7COBNb7X/xVZaHK9dlGHdlTpPgN0ZCl357JI
	 fQ+4ELCpIbZZ1WuVbkzubqbuVnzLuVQ7nFpWef9Y=
Received: by alsa1.perex.cz (Postfix, from userid 50401)
	id 34549F80588; Fri,  4 Aug 2023 11:43:03 +0200 (CEST)
Received: from mailman-core.alsa-project.org (mailman-core.alsa-project.org [10.254.200.10])
	by alsa1.perex.cz (Postfix) with ESMTP id 7771BF80579;
	Fri,  4 Aug 2023 11:43:03 +0200 (CEST)
Received: by alsa1.perex.cz (Postfix, from userid 50401)
	id 302D1F801D5; Wed,  2 Aug 2023 10:43:43 +0200 (CEST)
...
Received: from fd01.gateway.ufhost.com (fd01.gateway.ufhost.com
 [61.152.239.71])
	...
	(No client certificate requested)
	by alsa1.perex.cz (Postfix) with ESMTPS id 0699AF80425
	for <alsa-devel@alsa-project.org>; Wed,  2 Aug 2023 10:43:16 +0200 (CEST)
DKIM-Filter: OpenDKIM Filter v2.11.0 alsa1.perex.cz 0699AF80425
Received: from EXMBX165.cuchost.com (unknown [175.102.18.54])
	(using TLSv1 with cipher DHE-RSA-AES256-SHA (256/256 bits))
	(Client CN "EXMBX165", Issuer "EXMBX165" (not verified))
	by fd01.gateway.ufhost.com (Postfix) with ESMTP id 8C4298308;
	Wed,  2 Aug 2023 16:43:05 +0800 (CST)
...
Subject: [PATCH v1 3/5] ASoC: dwc: i2s: Add StarFive JH7110 SoC support
Date: Wed, 2 Aug 2023 16:42:59 +0800
Message-ID: <20230802084301.134122-4-xingyu.wu@starfivetech.com>
...
Message-ID-Hash: GHMH65J542YU7COBN6UBA65A5DRGTLXY
X-Message-ID-Hash: GHMH65J542YU7COBN6UBA65A5DRGTLXY
...
Archived-At: 
 <https://mailman.alsa-project.org/hyperkitty/list/alsa-devel@alsa-project.org/message/GHMH65J542YU7COBN6UBA65A5DRGTLXY/>
List-Archive: 
...
+
 ...
 {
 	u32 ch_reg;
@@ -267,6 +283,11 @@ static int dw_i2s_hw_params(struct snd_pcm_substream=
 *substream,
 		config->data_width =3D 16;
 		dev->ccr =3D 0x00;
 		dev->xfer_resolution =3D 0x02;
+		/* Set DMA buswidth on JH7110 */
+		if (dev->is_jh7110) {
+			dev->play_dma_data.dt.addr_width =3D DMA_SLAVE_BUSWIDTH_2_BYTES;
+			dev->capture_dma_data.dt.addr_width =3D DMA_SLAVE_BUSWIDTH_2_BYTES;
+		}
 		break;
=20
 	case SNDRV_PCM_FORMAT_S24_LE:
@@ -279,6 +300,11 @@ static int dw_i2s_hw_params(struct snd_pcm_substream=
 *substream,
 		config->data_width =3D 32;
 		dev->ccr =3D 0x10;
 		dev->xfer_resolution =3D 0x05;
+		/* Set DMA buswidth on JH7110 */
+		if (dev->is_jh7110) {
+			dev->play_dma_data.dt.addr_width =3D DMA_SLAVE_BUSWIDTH_4_BYTES;
+			dev->capture_dma_data.dt.addr_width =3D DMA_SLAVE_BUSWIDTH_4_BYTES;
+		}
 		break;
=20
 	default:
@@ -444,6 +470,7 @@ static int dw_i2s_set_tdm_slot(struct snd_soc_dai *cp=
u_dai,	unsigned int tx_mask
 }
=20
...
@@ -628,17 +655,43 @@ static int dw_configure_dai_by_pd(struct dw_i2s_dev=
 *dev,
=20
...
+		dev->capture_dma_data.pd.filter =3D pdata->filter;
+	} else {
+		/* Use platform data and use snd_dmaengine_dai_dma_data on JH7110 SoC =
*/
...
+			dev->play_dma_data.dt.addr_width =3D bus_widths[idx];
...
+			dev->capture_dma_data.dt.addr_width =3D bus_widths[idx];
...
@@ -680,6 +733,189 @@ static int dw_configure_dai_by_dt(struct dw_i2s_dev=
 *dev,
=20
 }
=20
...
+
 ...
 {
 	struct dw_i2s_dev *dev =3D snd_soc_dai_get_drvdata(dai);
@@ -712,15 +948,25 @@ static int dw_i2s_probe(struct platform_device *pde=
v)
...
=20
 	...
 	if (irq >=3D 0) {
@@ -779,7 +1025,7 @@ static int dw_i2s_probe(struct platform_device *pdev=
)
...
+	if (!pdata || dev->is_jh7110) {
 		if (irq >=3D 0) {
 			...
 			dev->use_pio =3D true;
@@ -821,8 +1067,36 @@ static void dw_i2s_remove(struct platform_device *p=
dev)
 }
=20
 #ifdef CONFIG_OF
