...
Received: from alsa0.perex.cz (alsa0.perex.cz [77.48.224.243])
	by mail.lfdr.de (Postfix) with ESMTPS id BD69313E5D
	for <lists+alsa-devel@lfdr.de>; Sun,  5 May 2019 10:04:14 +0200 (CEST)
Received: from alsa1.perex.cz (alsa1.perex.cz [207.180.221.201])
	(using TLSv1.2 with cipher AECDH-AES256-SHA (256/256 bits))
	(No client certificate requested)
	by alsa0.perex.cz (Postfix) with ESMTPS id 38A0F176E;
	Sun,  5 May 2019 10:03:24 +0200 (CEST)
DKIM-Filter: OpenDKIM Filter v2.11.0 alsa0.perex.cz 38A0F176E
DKIM-Signature: v=1; a=rsa-sha256; c=relaxed/simple; d=alsa-project.org;
	s=default; t=1557043454;
	bh=0Xm85CbKwBVSoeWPzx9zLT7po9DP0pYZXbTSc+GnFxg=;
	h=From:To:Date:In-Reply-To:References:Cc:Subject:List-Id:
	 List-Unsubscribe:List-Archive:List-Post:List-Help:List-Subscribe:
	 From;
	b=C1arBPmmH/feU7ZGI/jx06v8oV+TFeGMfR9xUhK/DScsr647lWBKsdsXxSE1B10c6
	 rsy/nPG5AwrEwvPz1ekdgA9/lL8EcrCrZClWsxV/th+Caie0nPab9EXwi9YuLaRqZP
	 xude6Wcifj6kn2X/gYBygsDriMBREzznsGJAyj50=
Received: from alsa1.perex.cz (localhost.localdomain [127.0.0.1])
	by alsa1.perex.cz (Postfix) with ESMTP id 26B97F89726;
	Sun,  5 May 2019 10:01:30 +0200 (CEST)
X-Original-To: alsa-devel@alsa-project.org
Delivered-To: alsa-devel@alsa-project.org
Received: by alsa1.perex.cz (Postfix, from userid 50401)
 id 2FD50F80726; Sun,  5 May 2019 10:01:10 +0200 (CEST)
...
Received: from mx1.suse.de (mx2.suse.de [195.135.220.15])
 (using TLSv1.2 with cipher AECDH-AES256-SHA (256/256 bits))
 (No client certificate requested)
 by alsa1.perex.cz (Postfix) with ESMTPS id 79D3EF89671
 for <alsa-devel@alsa-project.org>; Sun,  5 May 2019 10:01:00 +0200 (CEST)
DKIM-Filter: OpenDKIM Filter v2.11.0 alsa1.perex.cz 79D3EF89671
X-Virus-Scanned: by amavisd-new at test-mx.suse.de
Received: from relay2.suse.de (unknown [195.135.220.254])
 by mx1.suse.de (Postfix) with ESMTP id A8EABAEAA;
 Sun,  5 May 2019 08:00:59 +0000 (UTC)
From: Takashi Iwai <tiwai@suse.de>
To: Mark Brown <broonie@kernel.org>
Date: Sun,  5 May 2019 10:00:52 +0200
Message-Id: <20190505080053.20212-2-tiwai@suse.de>
X-Mailer: git-send-email 2.16.4
In-Reply-To: <20190505080053.20212-1-tiwai@suse.de>
References: <20190505080053.20212-1-tiwai@suse.de>
Cc: alsa-devel@alsa-project.org,
 Pierre-Louis Bossart <pierre-louis.bossart@linux.intel.com>
Subject: [alsa-devel] [PATCH v5 1/2] ASoC: Add support for Conexant CX2072X
	CODEC
...
index 000000000000..24da0efe50c3
--- /dev/null
...
+			cx2072x_reg_raw_write(to_i2c_client(codec->dev),
+					      CX2072X_EQ_B0_COEFF,
...
+
+	mutex_lock(&cx2072x->lock);
+	memcpy(ucontrol->value.bytes.data, cx2072x->classd_amp,
...
+	struct snd_soc_component *codec = snd_soc_kcontrol_component(kcontrol);
+	struct cx2072x_priv *cx2072x = snd_soc_component_get_drvdata(codec);
+	const __le16 *param = (const __le16 *)ucontrol->value.bytes.data;
+	int changed = 0;
+
+	mutex_lock(&cx2072x->lock);
+	if (!memcmp(cx2072x->classd_amp, param, CX2072X_CLASSD_AMP_LEN))
+		goto unlock;
+	memcpy(cx2072x->classd_amp, param, CX2072X_CLASSD_AMP_LEN);
+
+	/* Config Power Averaging */
+	regmap_write(cx2072x->regmap, CX2072X_ANALOG_TEST10,
+		     le16_to_cpu(param[0]));
+	regmap_write(cx2072x->regmap, CX2072X_CODEC_TEST20,
+		     le16_to_cpu(param[1]));
+	regmap_write(cx2072x->regmap, CX2072X_CODEC_TEST26,
+		     le16_to_cpu(param[2]));
+	changed = 1;
+ unlock:
+	mutex_unlock(&cx2072x->lock);
...
+
+	mutex_lock(&cx2072x->lock);
+	memcpy(ucontrol->value.bytes.data, cx2072x->plbk_drc,
...
+	struct snd_soc_component *codec = snd_soc_kcontrol_component(kcontrol);
+	struct cx2072x_priv *cx2072x = snd_soc_component_get_drvdata(codec);
+	const u8 *param = ucontrol->value.bytes.data;
+	int changed = 0;
+
+	mutex_lock(&cx2072x->lock);
+	if (!memcmp(param, cx2072x->plbk_drc, CX2072X_PLBK_DRC_PARM_LEN))
+		goto unlock;
+	memcpy(cx2072x->plbk_drc, param, CX2072X_PLBK_DRC_PARM_LEN);
...
new file mode 100644
index 000000000000..cc329e03cb34
--- /dev/null
