...
Received: from alsa0.perex.cz (alsa0.perex.cz [77.48.224.243])
	by mail.lfdr.de (Postfix) with ESMTPS id 212A37B17C1
	for <lists+alsa-devel@lfdr.de>; Thu, 28 Sep 2023 11:42:15 +0200 (CEST)
Received: from alsa1.perex.cz (alsa1.perex.cz [207.180.221.201])
	(using TLSv1.2 with cipher ADH-AES256-GCM-SHA384 (256/256 bits))
	(No client certificate requested)
	by alsa0.perex.cz (Postfix) with ESMTPS id 76CD3E88;
	Thu, 28 Sep 2023 11:41:24 +0200 (CEST)
DKIM-Filter: OpenDKIM Filter v2.11.0 alsa0.perex.cz 76CD3E88
DKIM-Signature: v=1; a=rsa-sha256; c=relaxed/simple; d=alsa-project.org;
	s=default; t=1695894134;
	bh=GKHU9VaWpsSHaqycAAA2NKr/3awrWCVf/N/gXgJ9t98=;
	h=From:To:Subject:Date:In-Reply-To:References:List-Id:List-Archive:
	 List-Help:List-Owner:List-Post:List-Subscribe:List-Unsubscribe:
	 From;
	b=TLwKd0rjVNDNXEH4XAj08SQ/wud4tim1trbGz3tIHXLJM7Qsu3IXd5TYZs5/L7cmd
	 uczlLGTXMDlwD6YT5Y8Zn0qCDAoHWyUjE0caxslgcFqMN1docHd7TJ4+arLfCGS9SX
	 hP/Vn4Mt8pjeTFCH9heOOgkTPS5pgLlXPzFsZfRM=
Received: by alsa1.perex.cz (Postfix, from userid 50401)
	id 03E3AF8060B; Thu, 28 Sep 2023 11:38:28 +0200 (CEST)
Received: from mailman-core.alsa-project.org (mailman-core.alsa-project.org [10.254.200.10])
	by alsa1.perex.cz (Postfix) with ESMTP id 9248EF805F8;
	Thu, 28 Sep 2023 11:38:27 +0200 (CEST)
Received: by alsa1.perex.cz (Postfix, from userid 50401)
	id 697C2F805F7; Thu, 28 Sep 2023 11:38:22 +0200 (CEST)
...
Received: from inva020.nxp.com (inva020.nxp.com [92.121.34.13])
	...
	(No client certificate requested)
	by alsa1.perex.cz (Postfix) with ESMTPS id 23A65F8007C
	for <alsa-devel@alsa-project.org>; Thu, 28 Sep 2023 11:37:51 +0200 (CEST)
DKIM-Filter: OpenDKIM Filter v2.11.0 alsa1.perex.cz 23A65F8007C
Received: from inva020.nxp.com (localhost [127.0.0.1])
	by inva020.eu-rdc02.nxp.com (Postfix) with ESMTP id C56831A1C84;
	Thu, 28 Sep 2023 11:37:50 +0200 (CEST)
Received: from aprdc01srsp001v.ap-rdc01.nxp.com
 (aprdc01srsp001v.ap-rdc01.nxp.com [165.114.16.16])
	by inva020.eu-rdc02.nxp.com (Postfix) with ESMTP id 33E6C1A059B;
	Thu, 28 Sep 2023 11:37:50 +0200 (CEST)
Received: from localhost.localdomain (shlinux2.ap.freescale.net
 [10.192.224.44])
	by aprdc01srsp001v.ap-rdc01.nxp.com (Postfix) with ESMTP id 25A881820F57;
	Thu, 28 Sep 2023 17:37:48 +0800 (+08)
...
Subject: [RFC PATCH v5 10/11] media: imx-asrc: Add memory to memory driver
Date: Thu, 28 Sep 2023 17:00:18 +0800
Message-Id: <1695891619-32393-11-git-send-email-shengjiu.wang@nxp.com>
...
Message-ID-Hash: F6BVXPME7DZFMCY4MZB4T6UC56IGNRMP
X-Message-ID-Hash: F6BVXPME7DZFMCY4MZB4T6UC56IGNRMP
...
Archived-At: 
 <https://mailman.alsa-project.org/hyperkitty/list/alsa-devel@alsa-project.org/message/F6BVXPME7DZFMCY4MZB4T6UC56IGNRMP/>
List-Archive: 
...
index 000000000000..dd0be1761e0a
--- /dev/null
...
+		.format = SNDRV_PCM_FORMAT_IEC958_SUBFRAME_LE,
+	},
+};
+
...
+		new_rate = ctrl->p_new.p_asrc_rate->rate_integer;
+		new_frac = ctrl->p_new.p_asrc_rate->rate_fractional;
+		src_rate_int = asrc_check_rate(pair_m2m, IN, new_rate);
+		if (src_rate_int != new_rate ||
+		    (pair_m2m->src_rate.rate_integer > 0 &&
+		     src_rate_int != pair_m2m->src_rate.rate_integer))
+			return -EINVAL;
+
+		pair->rate[V4L_OUT] = src_rate_int;
+
+		if (new_frac != pair_m2m->src_rate.rate_fractional &&
+		    new_rate == pair_m2m->src_rate.rate_integer &&
+		    pair_m2m->dst_rate.rate_integer > 0) {
+			/*
+			 * use maximum rate 768kHz as limitation, then we can shift right 21 bit for
+			 * division
+			 */
+			src_rate_frac = pair_m2m->src_rate.rate_fractional;
+			src_rate = ((s64)src_rate_int << 31) + src_rate_frac;
+			dst_rate_int = pair_m2m->dst_rate.rate_integer;
+			dst_rate_frac = pair_m2m->dst_rate.rate_fractional;
...
+		}
+
+		pair_m2m->src_rate.rate_integer = new_rate;
+		pair_m2m->src_rate.rate_fractional = new_frac;
+
+		break;
...
+
+		if (new_frac != pair_m2m->dst_rate.rate_fractional &&
+		    new_rate == pair_m2m->dst_rate.rate_integer &&
+		    pair_m2m->src_rate.rate_integer > 0) {
+			/*
+			 * use maximum rate 768kHz as limitation, then we can shift right 21 bit for
+			 * division
...
+		}
+
+		pair_m2m->dst_rate.rate_integer = new_rate;
+		pair_m2m->dst_rate.rate_fractional = new_frac;
+
+		break;
+	default:
+		ret = -EINVAL;
...
+	f->fmt.audio.pixelformat = find_fourcc(pair->sample_format[V4L_CAP]);
+
+	return 0;
+}
+
...
+	f->fmt.audio.pixelformat = find_fourcc(pair->sample_format[V4L_OUT]);
+
+	return 0;
+}
+
...
+	struct device *dev = &m2m->pdev->dev;
+
+	f->fmt.audio.pixelformat = asrc_check_format(pair_m2m, OUT, f->fmt.audio.pixelformat);
...
+	pair->sample_format[V4L_CAP] = find_format(f->fmt.audio.pixelformat);
+
+	return 0;
+}
+
...
+	struct device *dev = &m2m->pdev->dev;
+
+	f->fmt.audio.pixelformat = asrc_check_format(pair_m2m, IN, f->fmt.audio.pixelformat);
...
+	pair_m2m->channels[V4L_OUT] = f->fmt.audio.channels;
+	pair->channels = f->fmt.audio.channels;
+	pair->sample_format[V4L_OUT] = find_format(f->fmt.audio.pixelformat);
+
+	return 0;
+}
+
