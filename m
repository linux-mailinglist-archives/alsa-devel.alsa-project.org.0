...
Received: from alsa0.perex.cz (alsa0.perex.cz [77.48.224.243])
	by mail.lfdr.de (Postfix) with ESMTPS id 5A2B16CA1C2
	for <lists+alsa-devel@lfdr.de>; Mon, 27 Mar 2023 12:53:56 +0200 (CEST)
Received: from alsa1.perex.cz (alsa1.perex.cz [207.180.221.201])
	(using TLSv1.2 with cipher ADH-AES256-GCM-SHA384 (256/256 bits))
	(No client certificate requested)
	by alsa0.perex.cz (Postfix) with ESMTPS id B273F820;
	Mon, 27 Mar 2023 12:53:04 +0200 (CEST)
DKIM-Filter: OpenDKIM Filter v2.11.0 alsa0.perex.cz B273F820
DKIM-Signature: v=1; a=rsa-sha256; c=relaxed/simple; d=alsa-project.org;
	s=default; t=1679914434;
	bh=h2SAYpvxPg3/6jIeovMooO+0xIu0a+IXWZ/yT0VgPvI=;
	h=From:To:Subject:Date:CC:List-Id:List-Archive:List-Help:List-Owner:
	 List-Post:List-Subscribe:List-Unsubscribe:From;
	b=YPZIB2uzFjx1bCB3cIXdK7lvjBK3weODHEnuNnJaO6MLlR79rrWXE/zgXLfDEz+/1
	 7HXtk68CPptAVvL2N903I98ziljtQmJoc/pmUx8xxcqnt0yDKWu6v8jmOjiNOVQd7p
	 nowCmoUIr9hqe47hS/BPlgDxAgIuNWiNxmnV8XyQ=
Received: from mailman-core.alsa-project.org (mailman-core.alsa-project.org [10.254.200.10])
	by alsa1.perex.cz (Postfix) with ESMTP id 1A364F8024E;
	Mon, 27 Mar 2023 12:53:04 +0200 (CEST)
Received: by alsa1.perex.cz (Postfix, from userid 50401)
	id 3E595F80272; Mon, 27 Mar 2023 12:52:56 +0200 (CEST)
...
Received: from n169-111.mail.139.com (n169-111.mail.139.com [120.232.169.111])
	...
	(No client certificate requested)
	by alsa1.perex.cz (Postfix) with ESMTPS id 568BBF800C9
	for <alsa-devel@alsa-project.org>; Mon, 27 Mar 2023 12:52:36 +0200 (CEST)
DKIM-Filter: OpenDKIM Filter v2.11.0 alsa1.perex.cz 568BBF800C9
X-RM-TagInfo: emlType=0                                       
X-RM-SPAM: 
X-RM-SPAM-FLAG: 00000000
Received: from localhost.localdomain (unknown[183.194.156.192])
	by rmsmtp-lg-appmail-15-12004 (RichMail) with SMTP id 2ee464217568968-ff13f;
	Mon, 27 Mar 2023 18:52:27 +0800 (CST)
X-RM-TRANSID: 2ee464217568968-ff13f
From: Shenghao Ding <13916275206@139.com>
To: broonie@kernel.org,
	lgirdwood@gmail.com,
	perex@perex.cz,
	pierre-louis.bossart@linux.intel.com
Subject: [PATCH v8] ASoC: tas2781: Add tas2781 driver
Date: Mon, 27 Mar 2023 18:51:57 +0800
Message-Id: <20230327105157.21752-1-13916275206@139.com>
X-Mailer: git-send-email 2.34.1
MIME-Version: 1.0
Content-Transfer-Encoding: 8bit
Message-ID-Hash: PDPTVXXA4IZ6D7OSDRATCCC4ML5SNWIW
X-Message-ID-Hash: PDPTVXXA4IZ6D7OSDRATCCC4ML5SNWIW
...
Archived-At: 
 <https://mailman.alsa-project.org/hyperkitty/list/alsa-devel@alsa-project.org/message/PDPTVXXA4IZ6D7OSDRATCCC4ML5SNWIW/>
List-Archive: 
...
---
Changes in v8:
 - rework all the style issue
 - some kzalloc+memcpy was repalaced by kmemdup
 - rewrite check_inxxxxx_yram
 - Add commets on why tasdevice_dev_update_bits did not support
 - Add commets on why block size is not fixed.
 - avoid code duplication, add some new function
 - Use shorter name
...
---
 ...
 sound/soc/codecs/tas2781-dsp.c | 2140 ++++++++++++++++++++++++++++++++
 sound/soc/codecs/tas2781-dsp.h |  181 +++
 sound/soc/codecs/tas2781-i2c.c | 1618 ++++++++++++++++++++++++
 sound/soc/codecs/tas2781.h     |  159 +++
 6 files changed, 4113 insertions(+)
...
index 000000000000..15ca256416fa
--- /dev/null
...
+		goto out;
+	}
+	img_data->nr_blk = SMS_HTONL(data[offset], data[offset + 1],
+		data[offset + 2], data[offset + 3]);
+	offset += 4;
+
...
+	for (i = 0; i < tas_fmw->nr_programs; i++) {
+		config = &(tas_fmw->configs[i]);
+		config->cfg_size = SMS_HTONL(buf[offset], buf[offset + 1],
+			buf[offset + 2], buf[offset + 3]);
+		offset += 4;
+	}
+
...
+		goto out;
+	}
+	fw_fixed_hdr->fwsize = SMS_HTONL(buf[offset], buf[offset + 1],
+		buf[offset + 2], buf[offset + 3]);
+	offset += 4;
+	if (offset + 4 > fmw->size) {
+		dev_err(tas_dev->dev, "%s: File Size error\n", __func__);
...
+		goto out;
+	}
+	fw_fixed_hdr->ppcver = SMS_HTONL(buf[offset], buf[offset + 1],
+		buf[offset + 2], buf[offset + 3]);
+	offset += 4;
+	if (offset + 4 > fmw->size) {
...
+		goto out;
+	}
+	fw_fixed_hdr->drv_ver = SMS_HTONL(buf[offset], buf[offset + 1],
+		buf[offset + 2], buf[offset + 3]);
+	offset += 4;
+	if (offset + 4 > fmw->size) {
...
new file mode 100644
index 000000000000..9ec80a91cf3a
--- /dev/null
...
+	 * receiver, games, audio-to-haptics, bypass mode, portrait, landscape,
+	 * etc. Even in multiple audio, one or two of the chips should work for
+	 * special cases. In order to support these variable-numbers of audio
+	 * cases, flexible configs have been introduced in the dsp firmware.
+	 */
...
+
+		}
+		bk_da[i]->yram_checksum = SMS_HTONS(config_data[config_offset],
+			config_data[config_offset + 1]);
+		config_offset += 2;
+		bk_da[i]->block_size = SMS_HTONL(config_data[config_offset],
+			config_data[config_offset + 1],
+			config_data[config_offset + 2],
+			config_data[config_offset + 3]);
+		config_offset += 4;
+
+		bk_da[i]->n_subblks = SMS_HTONL(config_data[config_offset],
+			config_data[config_offset + 1],
+			config_data[config_offset + 2],
+			config_data[config_offset + 3]);
+
...
+
+	ret = snd_soc_add_component_controls(tas_dev->codec,
+		dsp_ctrls, nr_controls < mix_index ? nr_controls : mix_index);
+
+out:
+	return ret;
+}
...
new file mode 100644
index 000000000000..5fdb241c4e5a
--- /dev/null
